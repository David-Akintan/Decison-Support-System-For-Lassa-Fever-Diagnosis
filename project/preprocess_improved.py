# preprocess_improved.py - Enhanced preprocessing with data quality improvements
import pandas as pd
import numpy as np
...
def create_improved_graph(X, k=8, similarity_threshold=0.1):
    """Create improved graph structure with better connectivity"""
    print(f"=== Creating Improved Graph Structure ===")
    n_samples = X.shape[0]
...
def df_to_pyg_data_improved(csv_path, k=8):
    ...
    # Create graph
    edge_index, edge_weights = create_improved_graph(processed['X'], k=k)
