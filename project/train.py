...
from models import GCNNet, GATNet, HybridGNN, AdvancedHybridGNN, GCNNet, GATNet
    ...
    for epoch in range(epochs):
        ...
        if epoch % 5 == 0:
            ...
            with torch.no_grad():
                ...
                scheduler.step(val_acc)  # Use validation accuracy instead
                
                print(f"Epoch {epoch:3d} | Loss: {loss:.4f} | Val Acc: {val_acc:.4f} | "
                      f"Val F1: {val_f1:.4f} | Minority F1: {minority_f1:.4f}")
                
                # Early stopping on validation accuracy (more stable)
                if val_acc > best_f1:
                    best_f1 = val_acc
                    patience_counter = 0
